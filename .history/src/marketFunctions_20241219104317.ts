...
import { Fruit, FruitsStock, OrderItem } from "./marketTypes";
...
export const removeFruitFromStock = (
  stock: FruitsStock,
  fruitId: number
): FruitsStock => {
...
import { Fruit, FruitsStock, State } from "./marketTypes";
import { Option, Some, None, some, none } from "./optionTypes";

// Retourne un fruit par nom
export const readFruitByName = (state: State, name: string): Option<Fruit> => {
  const fruit = state.fruitsStock.find((f) => f.name === name);
  return fruit ? some(fruit) : none();
};
...
export const calculateStockAmount = (stock: FruitsStock): number => {
  return stock.reduce(
