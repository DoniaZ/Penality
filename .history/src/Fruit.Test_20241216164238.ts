...
describe('Market Functions', () => {
  ...
  beforeEach(() => {
    state = {
      fruitsStock: [
        ...
        { id: 2, name: "Poire", availability: 50, price: 2.30 },
        { id: 3, name: "Ananas", availability: 30, price: 2.90 },
      ],
    };
  });
  ...
  it('should calculate the initial stock value', () => {
    ...
    expect(stockValue).toBeCloseTo(100 * 1.22 + 50 * 2.30 + 30 * 2.90);
  });

  it('should add 30 strawberries to the stock', () => {
    const strawberries: Fruit = { id: 4, name: "Fraise", availability: 30, price: 7.00 };
  ...
  it('should find a fruit by name "Fraise"', () => {
    ...
    expect(result).toEqual(some({ id: 4, name: "Fraise", availability: 30, price: 7.00 }));
  });
  ...
  it('should remove all strawberries from the stock', () => {
    ...
    expect(state.fruitsStock.find(fruit => fruit.name === "Fraise")).toBeUndefined();
  });
  ...
  it('should update the availability of pineapples to 10', () => {
    ...
    expect(state.fruitsStock.find(fruit => fruit.name === "Ananas")?.availability).toBe(10);
  });

  it('should sell 5 pineapples', () => {
    state.fruitsStock = sellFruit(state.fruitsStock, 3, 5);
    expect(state.fruitsStock.find(fruit => fruit.name === "Ananas")?.availability).toBe(5);
  });

  it('should add 10 melons to the stock', () => {
    const melons: Fruit = { id: 5, name: "Melon", availability: 10, price: 10.00 };
  ...
  it('should sell 2 strawberries, 1 melon, and 1 pineapple', () => {
    ...
    expect(state.fruitsStock.find(fruit => fruit.name === "Fraise")?.availability).toBe(28);
    expect(state.fruitsStock.find(fruit => fruit.name === "Melon")?.availability).toBe(7);
    expect(state.fruitsStock.find(fruit => fruit.name === "Ananas")?.availability).toBe(-6);
  });

  it('should calculate the value of the sale', () => {
    const saleValue = calculateStockAmount(state.fruitsStock);
    expect(saleValue).toBeCloseTo(28 * 7.00 + 7 * 10.00 - 6 * 2.90);
  });
