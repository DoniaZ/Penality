import { Either, left, right } from "./eitherither";
import { none } from "./Option";
import { Fruit, FruitsStock, OrderItem } from "./types";
...
const readFruitById = (stock: FruitsStock, fruitId: number): Option<Fruit> => {
  return stock.find((fruit) => fruit.id === fruitId) || none();
};

// Retourne un fruit par nom
const readFruitByName = (stock: FruitsStock, name: string): Option<Fruit> => {
  return stock.find((fruit) => fruit.name === name) || none();
};
...
const sellFruit = (
  ...
  fruitId: number,
  quantity: number
): Either<Error, FruitsStock> => {
  const fruit = stock.find((fruit) => fruit.id === fruitId);
  if (!fruit || fruit.availability < quantity) {
    return left(new Error("Insufficient stock or fruit not found"));
  }
  return right(
    stock.map((fruit) =>
      fruit.id === fruitId
